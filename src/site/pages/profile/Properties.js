...
import { fetchUserProperties,deleteUserAdditionalAddress} from '../../../redux/actionCreators/appActionCreators';
import { isAppEmbedWebview,checkPrimaryHolder } from '../../../utils/utils';
...
const UserProperties = () => {
    ...
    const isUserPaid=()=>{
        if(currentUser.iapRecipt!=='null') return true;
        const whiteList =[
            '164756803881',
        ];
        if(whiteList.includes( currentUser.mobileNumber)) return true;
        return false;
    }
    ...
    return (
        <Card id='propertiesList'>
                     ...
                     <Table striped>
                        ...
                        <tbody>
                            {properties.map((userProperty) => (
                                <>
                                    <tr>
                                        <td>
                                            <b>{ userProperty.id === currentUser.property.id ?'Main Registered  Address ' + ( userProperty.totalOccupants!=null && userProperty.totalOccupants!=undefined? '(' +userProperty.totalOccupants+')':'' ):  userProperty.primaryHolder?'Primary Address ' + '(' +userProperty.totalOccupants+')':'Secondary Address'} </b> <br/>
                                     ...
                                     { userProperty.id === currentUser.property.id?null:       <Button
                        ...
                        <tbody>
                            {properties.map((userProperty) => (
                                <>
                                    <tr>
                                        <td> <b>{ userProperty.id === currentUser.property.id ?'Main Registered  Address': userProperty.primaryHolder?'Primary':'Secondary'} </b></td>
            ...
            <CardFooter>
             { totalPropertyCount <= 7 ?  isAppEmbedWebview() ?  <Button
                    block
                    tag={Link}
                    color={'success'}
                    onClick={()=>{
                        if(isUserPaid()){
                          history.push(`/mobile-user-property/new`);
                        }else{
                    ...
                    onClick={()=>{
                          if(isUserPaid()){
                            history.push(`/profile/user-property/new`);
                          }else{
